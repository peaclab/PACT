...
def gridTemp2File(arr):
    global counter
    np.savetxt(gridSteadyFile+".layer"+str(counter),
               arr, delimiter="\n", fmt="%.2f")
    counter += 1
...
gridtransientFile = '.'.join(parser_args.gridSteadyFile.split('.')[
                             :-2])+'.block.transient.csv'
logFile = '.'.join(parser_args.gridSteadyFile.split('.')[:-2])+'.log'
SpiceFile = '.'.join(parser_args.gridSteadyFile.split('.')[:-2])+'.cir'
SpiceResultFile = '.'.join(
    parser_args.gridSteadyFile.split('.')[:-2])+'.cir.csv'
...
if(str(modelParams.get('Simulation', 'temperature_dependent')) == 'True'):
    ...
    deltaLayer = 1
    hybrid_wick_properties = MaterialProp_dict[(
        defaultConfigFile, "HybridWick")]
...
###Temperature-dependent simulation framework###
    while(deltaT > convergence):
        ...
        deltaT = np.max(
            abs(grid_temperature[deltaLayer] - grid_temperature_old))
        count += 1
        if(count == 100):
    ...
    with open(SpiceResultFile, "r")as myfile:
        for num, lines in enumerate(myfile):
            if num > 1:
                ...
                reshape = tmp.reshape(
                    int(num_layers), int(grid_rows), int(grid_cols))
                with open(gridtransientFile, "a") as myfile:
                    myfile.write("step "+str(num-1)+" ")
                # print(reshape)
                gridManager.grid2block(chipStack, reshape, modelParams.get(
                    'Grid', 'grid_mode'), transient=True, transientFile=gridtransientFile)
gridManager.grid2block(chipStack, grid_temperature,
                       modelParams.get('Grid', 'grid_mode'))
