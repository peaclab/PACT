...
class GridManager:
    def __init__(self,config_grid):
        ...
        #self.Lock = np.zeros((rows,cols))
        #self.Rx = np.zeros((rows,cols))
        #self.Ry = np.zeros((rows,cols))
        #self.Rz = np.zeros((rows,cols))
        #self.C = np.zeros((rows,cols))
        #self.I = np.zeros((rows,cols))
        #self.g2bmap = np.empty([int(self.grid_dict['rows']),int(self.grid_dict['cols'])],dtype="<U10")
        self.vector_evaluateRC = np.vectorize(self.evaluateRC,otypes=None, excluded = ['grid_length','grid_width','Rx','Ry','Rz','I','Lock'])
    ...
    def createGrids(self, chipstack,label_config_dict):
        ...
        #print(chipstack,label_config_dict)
        #sys.exit(2)
        #print("createGrids")
    ...
    def calculate_boundary_grids(self, layer_obj,grid_length,grid_width, num_layers):
        ...
        """
        Lock = np.zeros((rows,cols))
        Rx = np.zeros((rows,cols))
        Ry = np.zeros((rows,cols))
        Rz = np.zeros((rows,cols))
        ...
        #print(layer_obj.layer_num,num_layers-1)
        if(layer_num < num_layers-1):
            ...
            grid_left_x = np.floor(X_vals/grid_length).astype(int)
            grid_bottom_y= (rows - np.floor(Y_vals/grid_width) - 1).astype(int)
            ...
            grid_right_x = np.where(ceil==floor,ceil - 1,floor).astype(int)

            ceil = np.ceil(temp_y_vals)
            floor = np.floor(temp_y_vals)
            grid_top_y = np.where(ceil == floor,rows - floor,rows - ceil).astype(int)
            ...
            for (label,cfile) in self.label_config_dict.keys() :
                if(label in label_ll and self.label_config_dict[(label,cfile)] == None):
                    RCIout = {"Rx":0,"Ry":1,"Rz":0,"C":0,"I":0}
                
                #print (label,cfile)
                    if(self.label_mode_dict[label]=='single'):
                    #if(self.label_mode_dict[label]=='single'):
                        if(label=='Si'):
                            LibSi.defineGridProperties(grid_length,grid_width,thickness,self.config._sections['Si'], RCIout )
                            #print(RCIout)
                        ...
                        self.label_config_dict[(label,cfile)]=RCIout
            ...
            self.vector_evaluateRC(X_vals,Y_vals,length_vals, height_vals, PowerDensities,grid_left_x,grid_right_x,grid_bottom_y,grid_top_y,label_vals,flp_df['ConfigFile'].values,block_counter,grid_length = grid_length, grid_width = grid_width, Rx = Rx, Ry = Ry, Rz = Rz, I = I, Lock = Lock)
            ...
            if(str(layer_obj.LateralHeatFlow) == 'True'):
                ...
                layer_obj.Rx = np.ones(Rx.shape) * math.inf
                layer_obj.Ry = np.ones(Rx.shape) * math.inf
            ...
            if(layer_obj.LateralHeatFlow or layer_obj.VerticalHeatFlow):
                ...
                #print(self.label_config_dict.keys())
                #sys.exit(2)
                #NoPackage_info = LibNoPackage.defineGridProperties(grid_length,grid_width,thickness,self.config._sections['NoPackage'],RCIout)
                LibNoPackage.defineGridProperties(grid_length,grid_width,thickness,self.config._sections['NoPackage'],RCIout)
            ...
            if (str(layer_obj.LateralHeatFlow) == 'True'):
                ...
                layer_obj.Rx = np.ones(Rx.shape) * RCIout['Rx']
                layer_obj.Ry = np.ones(Ry.shape) * RCIout['Ry']
                ...
                layer_obj.Rx = np.ones(Rx.shape) * math.inf
                layer_obj.Ry = np.ones(Ry.shape) * math.inf


            #print(layer_obj.VerticalHeatFlow)
            if (str(layer_obj.VerticalHeatFlow) == 'True'):
                #print("noPackage layer has vertical TRUE")
                #r_amb = round(1/(float(self.config.get('NoPackage','htc'))*grid_length*grid_width) + NoPackage_info['Rz'],8)
                #r_amb = layer_obj.Rz + np.full((int(self.grid_dict['rows']),int(self.grid_dict['cols'])),r_amb)
                r_amb = 1/(float(self.config.get('NoPackage','htc'))*grid_length*grid_width) + RCIout['Rz']
                ...
                #r_amb = round(1/(float(self.config.get('NoPackage','htc'))*grid_length*grid_width),8)
            ...
            #print("Rz",layer_obj.Rz)
            #### Commented for speed - up: layer_obj.C = np.ones((int(self.grid_dict['rows']),int(self.grid_dict['cols'])),dtype=float)
    ...
    def evaluateRC(self, X,Y,length,width, PowerDensity,leftX,rightX,bottomY,topY,label,config,block_idx, grid_length, grid_width, Rx, Ry, Rz, I, Lock):
        ...
        if(mode =='matrix' and PowerDensity == 0):
            #print("matrix mode in evaluate RC with zero power density...EXITING")
            #self.Lock[topY:bottomY+1,leftX:rightX+1] +=1
            Lock[topY:bottomY+1,leftX:rightX+1] +=1
            return

        label_config_dict = self.label_config_dict[(label,config)]
        ...
        mode = self.label_mode_dict[label]
        length_lb_o = round((leftX+1)*grid_length - X,12)
        length_rb_o =  round((X+length) - (rightX)*grid_length,12)
        length_lt_o = length_lb_o
        length_rt_o = length_rb_o
        width_lb_o = round((int(self.grid_dict['rows']) - bottomY )*grid_width  - Y,12)
        width_rb_o = width_lb_o
        width_lt_o = round((topY + 1) * grid_width  - (self.width - Y - width),12)
        width_rt_o = width_lt_o
        #print(length_lb_o, length_rb_o, grid_length)
        #print(width_lb_o, width_lt_o, grid_width)
        #sys.exit(0)
        ...
        if((length_lb_o == length_rb_o == grid_length) and (width_lb_o == width_lt_o == grid_width)):
            ...
            #self.Lock[topY:bottomY+1,leftX:rightX+1] +=1
            I[topY:bottomY+1,leftX:rightX+1] += power #+ self.label_config_dict[(label,config)]['I']
            Lock[topY:bottomY+1,leftX:rightX+1] +=1
            if (mode == 'single'):
                #self.Rz[topY:bottomY+1,leftX:rightX+1]= np.reciprocal( a +  mask * (1/self.label_config_dict[(label,config)]['Rz']))
                Rx[topY:bottomY+1,leftX:rightX+1]= label_config_dict['Rx']
                Ry[topY:bottomY+1,leftX:rightX+1]= label_config_dict['Ry']
                Rz[topY:bottomY+1,leftX:rightX+1]= label_config_dict['Rz']
                #print(Rx,Ry,Rz)
            ...
            print("Boundaries are shared in this grid configuration")
            ...
            #sys.exit(2)
            I[topY:bottomY+1,leftX:rightX+1] += (mask * power)
            
            Lock[topY:bottomY+1,leftX:rightX+1] += mask
            ...
            if(mode =='single'):
                ...
                a = Rx[topY:bottomY+1,leftX:rightX+1]
                a = np.reciprocal(a, where = a !=0, out = np.zeros_like(a))
                Rx[topY:bottomY+1,leftX:rightX+1]= np.reciprocal( a +  mask * (1/label_config_dict['Rx']))
                ...
                b = Ry[topY:bottomY+1,leftX:rightX+1]
                b = np.reciprocal(b, where = b !=0, out = np.zeros_like(b))
                Ry[topY:bottomY+1,leftX:rightX+1]= np.reciprocal( b +  mask * (1/label_config_dict['Ry']))

                #a = self.Rz[topY:bottomY+1,leftX:rightX+1]
                #a = np.reciprocal(a, where = a !=0, out = np.zeros_like(a))
                #self.Rz[topY:bottomY+1,leftX:rightX+1]= np.reciprocal( a +  mask * (1/self.label_config_dict[(label,config)]['Rz']))

                c = Rz[topY:bottomY+1,leftX:rightX+1]
                c = np.reciprocal(c, where = c !=0, out = np.zeros_like(c))
                Rz[topY:bottomY+1,leftX:rightX+1]= np.reciprocal( c +  mask * (1/label_config_dict['Rz']))
            ###DEBUG
            """
            if(block_idx==4):
                print(self.Rx)
