...
#pr.enable()
water_lr = load('/projectnb/peaclab-cri-cooling/EDAToolDevelopment/MLModels/water_regression.joblib')
water_scale =load('/projectnb/peaclab-cri-cooling/EDAToolDevelopment/MLModels/water_scaler.joblib')
r245fa_lr = load('/projectnb/peaclab-cri-cooling/EDAToolDevelopment/MLModels/r245fa_regression.joblib')
r245fa_scale =load('/projectnb/peaclab-cri-cooling/EDAToolDevelopment/MLModels/r245fa_scaler.joblib')
r141b_lr = load('/projectnb/peaclab-cri-cooling/EDAToolDevelopment/MLModels/r141b_regression.joblib')
r141b_scale =load('/projectnb/peaclab-cri-cooling/EDAToolDevelopment/MLModels/r141b_scaler.joblib')
#pr.disable()
#s = io.StringIO()
#sortby = 'cumulative'
...
def defineGridPropertiesMatrix(label,width, height,thickness,properties):
    ...
    l = len(T)
    coolant_df = pd.DataFrame({'t [um]':[t]*l,'dp [um]':[dp]*l,'phi':[phi]*l,'AR':[ar]*l,'SF':[sf]*l,'w [um]':[w]*l,'T':[float(item)-323.15 for item in T ]})
    if coolant=='water':
