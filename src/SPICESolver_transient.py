                ...
                for grididx in range(self.size):
                    ...
                    if layer!= self.layer_limit and layer!=self.heatspreader and self.I_avg[layer][row][col]!=0:
                        ...
                        if len(self.I[layer])>1:
                            ...
                            text+=f" {(i)*float(res[0])}{res[1]} 0A {(i)*float(res[0])+1e-12}{res[1]} {self.I[layer][i-1][row][col]}A"
                            i+=1
                            while i<=len(self.I[layer]):
                                temp = re.compile("([0-9.]+)([a-zA-Z]+)")
                                res = temp.match(self.ptrace_step_size).groups()
                                text+=f" {(i)*float(res[0])}{res[1]} {self.I[layer][i-2][row][col]}A {(i)*float(res[0])+1e-12}{res[1]} {self.I[layer][i-1][row][col]}A"
                                i+=1
                            text+=")\n"
                            myfile.write(text)
