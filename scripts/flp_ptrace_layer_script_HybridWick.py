...
######Floorplan###############
fname0 = "../flp_files/Tests_withHotSpot/L0_hybridWick.csv"
fpartial0 = "Tests_withHotSpot/L0_hybridWick.csv"
fname1 = "../flp_files/Tests_withHotSpot/L1_hybridWick.csv"
fpartial1 = "Tests_withHotSpot/L1_hybridWick.csv"
...
for p_num, PD in enumerate(powerdensity):
    ...
    print(PD,"W/cm2")
    print("power",power)
    pname = "../ptrace_files/Tests_withHotSpot/"+chiplabel+"_hybridWickUniformPD_"+str(PD)+"Wcm2_ptrace.csv"
    ppartial = "Tests_withHotSpot/"+chiplabel+"_hybridWickUniformPD_"+str(PD)+"Wcm2_ptrace.csv"
    for idx, i in enumerate(range(total_blocks)):
        uname="Block_"+str(i)
    ...
    lname = "../lcf_files/Tests_withHotSpot/hybridWick_lcf_UniformPD_"+str(PD)+"Wcm2.csv"
    print(lname,pname)
...
for bg in bgpd:
    for hs in hspd:
        ...
        df = pd.DataFrame([],columns=['UnitName','Power'])
        #print(PD,"W/cm2")
        pname = "../ptrace_files/Tests_withHotSpot/"+chiplabel+"_hybridWickNonUniformPD_"+str(bg)+"_"+str(hs)+"Wcm2_ptrace.csv"
        ppartial = "Tests_withHotSpot/"+chiplabel+"_hybridWickNonUniformPD_"+str(bg)+"_"+str(hs)+"Wcm2_ptrace.csv"
        for idx, i in enumerate(range(total_blocks)):
            uname="Block_"+str(i)
        ...
        lname = "../lcf_files/Tests_withHotSpot/hybridWick_lcf_NonUniformPD_"+str(bg)+"_"+str(hs)+"Wcm2.csv"
        print(lname,pname)
        df1 = pd.DataFrame([],columns=['Layer','FloorplanFile','Thickness (m)','PtraceFile','LateralHeatFlow'])
        ff1 = pd.DataFrame([[0,fpartial0,0.0001,ppartial,True]],columns=['Layer','FloorplanFile','Thickness (m)','PtraceFile','LateralHeatFlow'])
        df1 = df1.append(ff1)
