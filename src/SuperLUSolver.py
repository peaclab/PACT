...
class SuperLUSolver:
    ...
    def getTemperature(self, dict_properties, mode=None):
        ...
        for grididx in range(self.size):
            ...
            curidx += 1
        A_csc = csc_matrix(
            (self.cooData, (self.cooX, self.cooY)), shape=(self.size, self.size))
        nn = 0
        lu = sla.splu(A_csc, permc_spec='MMD_AT_PLUS_A', diag_pivot_thresh=0.001,
                      options=dict(Fact='DOFACT', Equil=False, SymmetricMode=True))
        x = lu.solve(self.b)
