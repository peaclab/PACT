...
class SPICE_transientSolver:
    ...
    def update(self):
        ...
        for (layer, prop) in self.dict_properties_update['update'].items():
            if(prop == 'Rz'):
                self.dict_properties['Rz'].update(
                    self.dict_properties_update["Rz"])
                self.Rz.update(self.dict_properties_update['Rz'])
            elif(prop == 'Rx'):
                self.dict_properties['Rx'].update(
                    self.dict_properties_update["Rx"])
                self.Rx.update(self.dict_properties_update['Rx'])
            elif(prop == 'Ry'):
                self.dict_properties['Ry'].update(
                    self.dict_properties_update["Ry"])
                self.Ry.update(self.dict_properties_update['Ry'])
            elif(prop == 'C'):
                self.dict_properties['C'].update(
                    self.dict_properties_update["C"])
                self.C.update(self.dict_properties_update['C'])
            elif(prop == 'I'):
                self.dict_properties['I'].update(
                    self.dict_properties_update["I"])
    ...
    def setup(self):
        ...
        self.addY = {0: 1, 1: -1, 2: -self.nc, 3: self.nc, 4: -self.nr*self.nc,
                     5: self.nr*self.nc}  # 0...6 correspond to [Re,Rw,Rn,Rs,Ra,Rb]
    ...
    def getTemperature(self, dict_properties, logFile=None, SpiceFile=None, SpiceResultFile=None, mode=None):
        ...
        with open(SpiceFile, 'w') as myfile:
            ...
            for grididx in range(self.size):
                ...
                if (col > 0):
                    Rw = self.Rx[layer][row][col-1] / \
                        2 + self.Rx[layer][row][col]/2
                else:
                    Rw = 100000000
                if(col < (self.col_limit)):
                    Re = self.Rx[layer][row][col+1] / \
                        2 + self.Rx[layer][row][col]/2
                else:
                    Re = 100000000
                if(row > 0):
                    Rn = self.Ry[layer][row-1][col] / \
                        2 + self.Ry[layer][row][col]/2
                else:
                    Rn = 10000000
                if(row < self.row_limit):
                    Rs = self.Ry[layer][row+1][col] / \
                        2 + self.Ry[layer][row][col]/2
                else:
                    Rs = 10000000
                if(layer > 0):
                    Ra = float(self.factorVN[self.layerVN[layer-1]])*self.Rz[layer-1][row][col] + \
                        (1-float(self.factorVN[self.layerVN[layer]])
                         )*self.Rz[layer][row][col]
                else:
                    Ra = 100000000

                if(layer < self.layer_limit):
                    Rb = float(self.factorVN[self.layerVN[layer]])*self.Rz[layer][row][col] + \
                        (1-float(self.factorVN[self.layerVN[layer+1]])
                         )*self.Rz[layer+1][row][col]
                elif layer == self.heatsink:
                    Rb = float(
                        self.factorVN[self.layerVN[layer]])*self.Rz[layer][row][col]
                else:
                    Rb = 100000000
                # current:
                if layer != self.layer_limit and layer != self.heatspreader and self.I_avg[layer][row][col] != 0:
                    ...
                    if len(self.I[layer]) > 1:
                        text = "I_{}_{}_{} GND Node{}_{}_{} PWL(0s 0A".format(
                            layer, row, col, layer, row, col)
                        temp = re.compile("([0-9.]+)([a-zA-Z]+)")
                        res = temp.match(self.ptrace_step_size).groups()
                        step_size = float(temp.match(
                            self.step_size).groups()[0])
                        ptrace_step_size = float(temp.match(
                            self.ptrace_step_size).groups()[0])
                        if step_size < ptrace_step_size:
                            ...
                            raise Exception(
                                "Error: solver step size is larger than power step size")
                        ...
                        myfile.write("I_{}_{}_{} GND Node{}_{}_{} PULSE(0 {}A 0s 0s 0s {} {})\n".format(
                            layer, row, col, layer, row, col, self.I[layer][0][row][col], self.total_time, self.total_time))
                # east resistance
                if col != self.col_limit:
                    myfile.write("R_{}_{}_{}_1 Node{}_{}_{} Node{}_{}_{} {}\n".format(
                        layer, row, col, layer, row, col, layer, row, col+1, Re))
                # Heat Spreader right
                if col == self.col_limit and layer == self.heatspreader:
                    myfile.write("Rsp_{}_{}_{}_1 Node{}_{}_{} Node_sp_right {}\n".format(
                        layer, row, col, layer, row, col, self.Rx[layer][row][col]/2+self.row_limit*self.heatspreader_others['r_sp1_x_constant']))
                # Heat Sink right
                if col == self.col_limit and layer == self.heatsink:
                    myfile.write("Rhs_{}_{}_{}_1 Node{}_{}_{} Node_hs_right {}\n".format(
                        layer, row, col, layer, row, col, self.Rx[layer][row][col]/2+self.row_limit*self.heatsink_others['r_hs1_x_constant']))
                # Heat Spreader left
                ...
                if row != self.row_limit:
                    ...
                    if self.glabels[layer][row][col] != 'Liq':
                        myfile.write("R_{}_{}_{}_2 Node{}_{}_{} Node{}_{}_{} {}\n".format(
                            layer, row, col, layer, row, col, layer, row+1, col, Rs))
                    # liquid grid cell
                    else:
                        if row == 0:
                            # inlet
                ...
                if row == self.row_limit and layer == self.heatspreader:
                    myfile.write("Rsp_{}_{}_{}_2 Node{}_{}_{} Node_sp_top {}\n".format(layer, row, col, layer, row,
                                 col, self.Ry[layer][row][col]/2+self.col_limit*self.heatspreader_others['r_sp1_y_constant']))
                # Heat sink top
                if row == self.row_limit and layer == self.heatsink:
                    myfile.write("Rhs_{}_{}_{}_2 Node{}_{}_{} Node_hs_top {}\n".format(
                        layer, row, col, layer, row, col, self.Ry[layer][row][col]/2+self.col_limit*self.heatsink_others['r_hs1_y_constant']))
                # Heat spreader bot
                ...
                if layer != self.layer_limit:
                    myfile.write("R_{}_{}_{}_3 Node{}_{}_{} Node{}_{}_{} {}\n".format(
                        layer, row, col, layer, row, col, layer+1, row, col, Rb))
                elif layer != self.heatsink:
                    myfile.write("R_{}_{}_{}_3 Node{}_{}_{} GND {}\n".format(
                        layer, row, col, layer, row, col, self.r_amb))
                elif layer == self.heatsink:
                    myfile.write("R_{}_{}_{}_3 Node{}_{}_{} GND {}\n".format(
                        layer, row, col, layer, row, col, Rb))
                myfile.write("C_{}_{}_{} Node{}_{}_{} GND {}\n".format(
                    layer, row, col, layer, row, col, self.C[layer][row][col]))
            if len(self.heatsink_layer) != 0:
                # add heat spreader to heat sink inner node
                myfile.write(
                    f"R_sp_hs_in_left Node_sp_left Node_hs_in_left {self.heatspreader_others['r_sp_per_x_constant']}")
                myfile.write('\n')
                myfile.write(
                    f"R_sp_hs_in_right Node_sp_right Node_hs_in_right {self.heatspreader_others['r_sp_per_x_constant']}")
                myfile.write('\n')
                myfile.write(
                    f"R_sp_hs_in_top Node_sp_top Node_hs_in_top {self.heatspreader_others['r_sp_per_y_constant']}")
                myfile.write('\n')
                myfile.write(
                    f"R_sp_hs_in_bot Node_sp_bot Node_hs_in_bot {self.heatspreader_others['r_sp_per_y_constant']}")
                myfile.write('\n')
                # add heat hinfk inner to heat sink outter node
                myfile.write(
                    f"R_hs_hs_left Node_hs_in_left Node_hs_out_left {self.heatsink_others['r_hs2_x_constant']+self.heatsink_others['r_hs_constant']}")
                myfile.write('\n')
                myfile.write(
                    f"R_hs_hs_right Node_hs_in_right Node_hs_out_right {self.heatsink_others['r_hs2_x_constant']+self.heatsink_others['r_hs_constant']}")
                myfile.write('\n')
                myfile.write(
                    f"R_hs_hs_top Node_hs_in_top Node_hs_out_top {self.heatsink_others['r_hs2_y_constant']+self.heatsink_others['r_hs_constant']}")
                myfile.write('\n')
                myfile.write(
                    f"R_hs_hs_bot Node_hs_in_bot Node_hs_out_bot {self.heatsink_others['r_hs2_y_constant']+self.heatsink_others['r_hs_constant']}")
                myfile.write('\n')
                # add heat sink to ambient R
                myfile.write(
                    f"R_hs_amb_in_left Node_hs_in_left GND {self.heatsink_others['r_hs_c_per_x_constant']+self.heatsink_others['r_amb_c_per_x_constant']}")
                myfile.write('\n')
                myfile.write(
                    f"R_hs_amb_in_right Node_hs_in_right GND {self.heatsink_others['r_hs_c_per_x_constant']+self.heatsink_others['r_amb_c_per_x_constant']}")
                myfile.write('\n')
                myfile.write(
                    f"R_hs_amb_in_top Node_hs_in_top GND {self.heatsink_others['r_hs_c_per_y_constant']+self.heatsink_others['r_amb_c_per_y_constant']}")
                myfile.write('\n')
                myfile.write(
                    f"R_hs_amb_in_bot Node_hs_in_bot GND {self.heatsink_others['r_hs_c_per_y_constant']+self.heatsink_others['r_amb_c_per_y_constant']}")
                myfile.write('\n')
                myfile.write(
                    f"R_hs_amb_out_left Node_hs_out_left GND {self.heatsink_others['r_hs_per_constant']+self.heatsink_others['r_amb_per_constant']}")
                myfile.write('\n')
                myfile.write(
                    f"R_hs_amb_out_right Node_hs_out_right GND {self.heatsink_others['r_hs_per_constant']+self.heatsink_others['r_amb_per_constant']}")
                myfile.write('\n')
                myfile.write(
                    f"R_hs_amb_out_top Node_hs_out_top GND {self.heatsink_others['r_hs_per_constant']+self.heatsink_others['r_amb_per_constant']}")
                myfile.write('\n')
                myfile.write(
                    f"R_hs_amb_out_bot Node_hs_out_bot GND {self.heatsink_others['r_hs_per_constant']+self.heatsink_others['r_amb_per_constant']}")
                myfile.write('\n')

                # add capaciatance for extra package node
                myfile.write("C_sp_per_y_top Node_sp_top GND {}\n".format(
                    self.heatspreader_others['c_sp_per_y_constant']))
                myfile.write("C_sp_per_y_bot Node_sp_bot GND {}\n".format(
                    self.heatspreader_others['c_sp_per_y_constant']))
                myfile.write("C_sp_per_x_left Node_sp_left GND {}\n".format(
                    self.heatspreader_others['c_sp_per_x_constant']))
                myfile.write("C_sp_per_x_right Node_sp_right GND {}\n".format(
                    self.heatspreader_others['c_sp_per_x_constant']))
                myfile.write("C_hs_c_per_y_top Node_hs_in_top GND {}\n".format(
                    self.heatsink_others['c_hs_c_per_y_constant']))
                myfile.write("C_hs_c_per_y_bot Node_hs_in_bot GND {}\n".format(
                    self.heatsink_others['c_hs_c_per_y_constant']))
                myfile.write("C_hs_c_per_x_left Node_hs_in_left GND {}\n".format(
                    self.heatsink_others['c_hs_c_per_x_constant']))
                myfile.write("C_hs_c_per_x_right Node_hs_in_right GND {}\n".format(
                    self.heatsink_others['c_hs_c_per_x_constant']))
                myfile.write("C_hs_per_top Node_hs_out_top GND {}\n".format(
                    self.heatsink_others['c_hs_per_constant']))
                ...
                myfile.write("C_amb_per_top Node_hs_out_top GND {}\n".format(
                    self.heatsink_others['c_amb_per_constant']))
            ...
            myfile.write(
                f'.OPTIONS OUTPUT INITIAL_INTERVAL={self.step_size} {self.total_time}\n')
        ...
        if int(self.num_core) <= 1:
            ...
            os.system(
                f"mpirun -np {self.num_core} Xyce -l {logFile} {SpiceFile}")
        with open(SpiceResultFile, 'r') as myfile:
            tmp = np.asarray(
                list(map(float, list(myfile)[-1][:].split(',')[1:])))
